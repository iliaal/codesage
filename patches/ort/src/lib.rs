...
pub(crate) fn load_dylib_from_path(path: &std::path::Path) -> Result<bool> {
	let mut inserter = Some(|| -> crate::Result<libloading::Library> {
		...
		let lib = match unsafe { libloading::Library::new(&absolute_path) } {
			Ok(lib) => lib,
			Err(e) => {
				panic!("failed to load ONNX Runtime from `{}`: {e}", absolute_path.display())
			}
		};

		let base_getter: libloading::Symbol<unsafe extern "C" fn() -> *const ort_sys::OrtApiBase> =
			match unsafe { lib.get(b"OrtGetApiBase") } {
				Ok(sym) => sym,
				Err(_) => {
					panic!("expected `OrtGetApiBase` to be present in libonnxruntime at `{}`", absolute_path.display())
				}
			};
