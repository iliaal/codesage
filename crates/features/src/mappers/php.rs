//! Composer, PHP-extension, and Laravel feature mapping.
...
impl FeatureMapper for PhpMapper {
    ...
    fn map(&self, ctx: &MapperContext) -> Result<Vec<FeatureSeed>> {
        ...
        // Filter centrally so framework helpers need no MapperContext.
...
/// Map `ext/<name>/config.{m4,w32}` to extension features and linked `.phpt` tests.
fn php_src_extensions(root: &Path) -> Result<Vec<FeatureSeed>> {
    ...
    for sub_rel in list_dir_subdirs(root, &ext_dir, None) {
        ...
        // Prefer config.m4 for stable IDs; Windows-only extensions need config.w32.
        ...
        // Source ownership lets file-to-feature lookup find the extension; bound bundle size.
...
fn php_extension_api_source_files(root: &Path, dir: &Path) -> Vec<String> {
    // The orchestrator filters excluded paths before persistence.
...
/// Preserve route literals and byte offsets while blanking PHP comments.
...
/// Require `config.m4`/`config.w32` and `php_<name>.h` to distinguish extensions
/// from generic autotools projects. Emit an extension seed plus component seeds
/// for `<name>_<part>.{c,cpp,cc}`, with matching headers attached.
...
struct LaravelRoute {
    ...
    /// One-based registration line, carried onto synthesized RouteHandler references.
    line: u32,
}
...
fn parse_laravel_routes(root: &Path) -> Result<Vec<LaravelRoute>> {
    ...
    // Capture fluent calls before the verb so each registration inherits its prefixes.
    ...
    // Match only the group opener; php_block_end handles nested braces.
    ...
    // Controller groups supply the class; inner routes name only the action.
    let group_inner_re = Regex::new(
        r#"(?ms)Route::((?:[A-Za-z_]\w*\s*\((?:[^;{}'"]|'[^']*'|"[^"]*")*?\)\s*->\s*)*)(get|post|put|patch|delete|options|any)\s*\(\s*['"]([^'"]+)['"](?:\s*,\s*['"]([^'"]+)['"])?"#,
    )?;
    // Plain groups allow one nested options array (e.g. middleware => ['auth']).
    // Chain matches cannot cross closure bodies; php_block_end finds their extent.
    ...
    for file in ["web.php", "api.php", "console.php", "channels.php"] {
        ...
        let raw = read_to_string_bounded(&path)
            ...
            .flatten()
            .unwrap_or_default();
        // Comments must not create routes; preserve literal paths and match offsets.
...
fn scan_route_region(
    ...
    // Recurse only into outermost groups so nested prefixes compose once.
...
/// Map PHP import short names and aliases to FQCNs for controller matching.
...
/// Join normalized prefixes and URI, preserving the URI's leading-slash convention.
...
/// Link route files to handler methods using the parser's Namespace\Class\method
/// naming convention. Closures and unresolved class/action pairs emit no edge.
...
/// Map controllers to the routes that target them.
fn laravel_controllers(root: &Path, routes: &[LaravelRoute]) -> Result<Vec<FeatureSeed>> {
    ...
    for rel in files {
        ...
        // Use basename matching only when import resolution left a short class name.
...
/// Use the literal Artisan signature's command name, falling back to the class name.
...
/// Limit Composer script features to review/build commands and deploy* entries
/// to keep one-off pipeline scripts out of feature listings.
...
/// Give shared Laravel metadata a project-level owner alongside narrower features.
...
/// Group migrations into one configuration feature rather than one per file.
