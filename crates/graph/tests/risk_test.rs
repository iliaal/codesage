...
fn score_zero_and_note_when_no_git_history() {
    ...
    assert!(r.score < 0.2, "score without history should stay low, got {}", r.score);
    assert!(
...
fn hotspot_fix_heavy_file_scores_high_and_emits_notes() {
    ...
    let r = assess_risk(&db, "Repository.php").unwrap();
    assert!(r.score >= 0.6, "hotspot+fix-heavy should score >= 0.6, got {}", r.score);
    ...
    assert!(notes.contains("fix-heavy"), "missing fix-heavy note: {notes}");
}
...
fn cold_isolated_file_scores_low() {
    ...
    for p in ["Repository.php", "Controller.php", "Service.php"] {
        db.upsert_git_file(p, 10.0, 0, 5, Some(1_700_000_000)).unwrap();
    }
    db.upsert_git_file("other_cold.php", 0.01, 0, 1, Some(1_700_000_000))
        .unwrap();

    let r = assess_risk(&db, "other_cold.php").unwrap();
    assert!(r.score < 0.4, "cold file should score < 0.4, got {}", r.score);
...
fn test_gap_false_when_sibling_test_exists_without_coupling() {
    ...
    let r = assess_risk(&db, "Repository.php").unwrap();
    assert!(!r.test_gap, "sibling test file should close the test gap even without coupling");
}
...
fn high_coupling_triggers_coupling_note() {
    ...
    for i in 0..10 {
        ...
        db.upsert_git_file(&other, 0.5, 0, 5, Some(1_700_000_000)).unwrap();
        // "Repository.php" < "z_other_NN.php" lexicographically so the pair is sorted correctly.
        db.upsert_git_co_change("Repository.php", &other, (10 - i) as f64, 5, Some(1_700_000_000))
            .unwrap();
    }
...
fn wide_blast_radius_note_fires_when_many_dependents() {
    ...
    let r = assess_risk(&db, "Repository.php").unwrap();
    assert!(r.dependent_files < 10, "fixture has only 2 callers, got {}", r.dependent_files);
    assert!(
