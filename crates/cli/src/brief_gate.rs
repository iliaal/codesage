...
fn decide(state: &mut GateState, path: &str, payload: &str, now: u64) -> bool {
...
/// Should this payload be served for `path` in `session`?
///
/// Returns false on any I/O or parse failure, per the fail-closed rule above.
/// Recording is part of the answer: a true result has already been charged
/// against the budget, so the caller must serve what it asked about.
pub(crate) fn should_serve(dir: &Path, session: &str, path: &str, payload: &str) -> bool {
    if payload.is_empty() {
        return false;
    }
    ...
    if !decide(&mut state, path, payload, now) {
        return false;
    }
...
mod tests {
    ...
    fn the_same_payload_for_the_same_path_is_served_once() {
        let mut s = state();
        assert!(decide(&mut s, "a.rs", "hotspot: 90%", 1000));
        assert!(!decide(&mut s, "a.rs", "hotspot: 90%", 1000));
    ...
    fn other_paths_are_unaffected_by_a_paths_cooldown() {
        let mut s = state();
        assert!(decide(&mut s, "a.rs", "x", 1000));
        assert!(decide(&mut s, "b.rs", "x", 1000));
    }
    ...
    fn a_payload_larger_than_the_whole_budget_is_never_served() {
        ...
        assert!(!decide(&mut s, "a.rs", &huge, 1000));
    ...
    fn state_round_trips_through_the_runtime_dir() {
        ...
        let p = dir.path();
        assert!(should_serve(p, "sess-1", "a.rs", "hotspot: 90%"));
        assert!(!should_serve(p, "sess-1", "a.rs", "hotspot: 90%"));
        // A different session starts with its own budget and its own history.
        assert!(should_serve(p, "sess-2", "a.rs", "hotspot: 90%"));
        // An empty payload is never a fire.
        assert!(!should_serve(p, "sess-3", "a.rs", ""));
    }
    ...
    fn a_corrupt_state_file_starts_over_rather_than_disabling_the_gate() {
        ...
        assert!(should_serve(p, "sess", "a.rs", "x"));
        assert!(!should_serve(p, "sess", "a.rs", "x"));
    }
}
