...
5. Run `codesage-review-state changed-files --project "$PROJECT" --feature "$PROJECT/.codesage/reviews/$RUN_ID/features/<feature_id>.json" --target-ids "$PROJECT/.codesage/reviews/$RUN_ID/targets/<feature_id>.json" --output "$PROJECT/.codesage/reviews/$RUN_ID/changed/<feature_id>.json"`. The helper reads the feature's findings document and unions targeted finding paths with committed changes since `reviewed_at_sha` when present, staged, unstaged, and untracked slice paths. It uses project-relative, NUL-delimited Git output, excludes siblings of nested projects, and retains both rename endpoints. Without a prior SHA, it collects pending changes and target paths; outside Git it returns only the target paths. Collection errors and unavailable recorded commits fail the feature; never substitute an empty array.
