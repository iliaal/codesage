...
- JavaScript/TypeScript and C/C++ projects can gain import cycles and risk, TypeScript and Go projects can lose false ones, and a `session_start` snapshot taken before upgrading reports both at `session_end`.
- `recommend_tests` / `codesage tests-for` pick the JavaScript/TypeScript runner from the test's runner import or the nearest `package.json` and vitest config, run nested packages as `(cd -- ./<dir> && ...)`, and note tests with no runner evidence instead of guessing `npx jest`.
- The next `codesage index` reparses existing files under `parser-queries=4;extraction=7`, which the TypeScript, PHP, and C++ extraction changes need.
...
- A Go `import` or a bare JavaScript/TypeScript import specifier (`'debounce'`) no longer itself resolves to a same-named project symbol in `find_references` `to`, `impact_analysis`, `recommend_tests` reachability, `list_dependencies` `imported_by`, or import-cycle detection; a same-named import binding or call can still link the file.
...
- `recommend_tests` no longer blocks or reads unbounded input when a changed file or named test path is a FIFO, a device, or a symlink to one.
