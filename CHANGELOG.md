...
- `find_references` marks TypeScript type-only directives (`import type`, `export type { .. } from`, `import type X = require(..)`, clauses whose every specifier is `type`) `lazy: true`, and import-cycle detection skips them; the next `codesage index` reparses existing files under `extraction=7`.
- Import-cycle detection (`assess_risk`, `assess_risk_diff`, `review_rehearsal`, `session_start` / `session_end`) counts JavaScript/TypeScript relative specifiers and C/C++ quoted includes resolved to the one indexed file their resolver loads, ignoring `.d.ts` files.
- JavaScript and C projects can report new or larger import cycles and higher risk scores; a `session_start` snapshot taken before upgrading reports them as new at `session_end`.
- `recommend_tests` / `codesage tests-for` `commands[]` pick the JavaScript/TypeScript runner (vitest, jest, mocha, `node --test`, `bun test`, Playwright) from the test file's runner import or the nearest `package.json` and vitest config.
- A nested JavaScript package's test command runs as `(cd -- ./<dir> && ...)`, and a test with no runner evidence gets a note instead of an `npx jest` command.

### Fixed

- `codesage git-index` indexes the same history when `log.showSignature=true` or `log.showRoot=false`, and fails instead of reporting success when it cannot parse HEAD's commit date or `git log` output.
...
- `list_dependencies` no longer links a `.h` include to a same-named `.hpp`, an extensionless JavaScript import to a `.py` or `.h` file, or a bare JavaScript package specifier to a project file.
...
- `list_features` limit 0 is now unbounded (was silently reset to 100); other tools keep prior zero semantics, documented in descriptions.
...
- A model's HuggingFace artifact paths resolve once per process instead of on every hf-hub lookup in every MCP semantic query. (cs-u0e)
...
- The semantic fingerprint is format v4 and covers the ONNX Runtime: on Linux the loaded `libonnxruntime.so` joins the model artifacts (digest and stat key), on macOS the statically linked runtime's build info is digested; every existing chunk table re-embeds once on its next `index`, and a Linux host where no runtime is located refuses to fingerprint instead of attesting without it.
...
- The live watcher re-queues paths whose semantic pass failed (embedder load, DB open, embedding) with a doubling delay for up to five retries, logged at WARN, instead of dropping them until the next filesystem event; a bulk pass whose embedder fails to load reports `Failed` and keeps its paths for that retry instead of clearing them as done.
...
- `search` and `find_symbol` annotate an empty result with `_meta.coverage`: indexed file count and per-language breakdown. A language missing from the breakdown was never indexed, which an empty result alone cannot tell you.
- `find_references` rows can carry kind `import_binding`: the symbol an import introduces (`Foo`), alongside the `import` row naming the module (`./foo.js`). Covers re-exports, CommonJS destructuring with aliases, and a barrel unwrapping symbols off a value (`const { Foo } = axios`). Needs `codesage index --full`.

### Changed
- `search` drops the leading namespace components of a `::`- or `\`-qualified query name instead of OR-joining them. `Illuminate\Routing\Router` no longer lets two namespace terms outvote the class. A lowercase tail (`ModuleRef::create`) keeps its prefix. Dotted names are unchanged.
...
- `impact_analysis` finds JavaScript and TypeScript dependents that import a symbol and use it as `Foo.staticMethod()`, `x instanceof Foo`, or `obj.Foo = Foo`. Previously only the module specifier was recorded and those files dropped out. On axios, mean recall rose from 0.37 to 0.66 at precision 0.85.
- `impact_analysis` resolves JavaScript, TypeScript and C/C++ imports written as file paths (`./headers.js`, `dir/foo.h`). Previously only Rust-style `::` paths matched: in axios, `AxiosError` reported 0 dependents instead of 23. `assess_risk` blast radius and `export_context` callers read the same resolution.
- `impact_analysis --symbol` accepts a bare name whose definitions all share one qualified name, instead of failing with `qualify with one of: Foo, Foo`. Distinct qualified names still require disambiguation.
...
- **README headline updated to the semble-corpus number** (recall@10 = 0.932, NDCG@10 = 0.788 on 602 queries across 8 supported languages; 0.448 / 0.379 on the full 1,251-query corpus with the parser-coverage gap accounted for). The pre-existing git-mined-corpus benchmark stays as-is.
...
- SQLite `busy_timeout=5000` is now set on every database connection in `init_db`. A second MCP session reading mid-index waits up to 5 s for the lock instead of failing instantly with `SQLITE_BUSY`. Concurrent writers were already serialized by the advisory lockfile from §2.4; this closes the brief WAL-checkpoint windows that affected readers. Perf-neutral: it's a wait threshold, not a throughput knob, and a no-op when there's no contention. Matches repowise's hardening (recommendations doc §1.8). `journal_mode=WAL` and `foreign_keys=ON` were already set.
...
- `recommend_tests` for `.c` / `.h` source files now lists `.phpt` tests in `<dir>/tests/` as primary (php-src convention). Skips when the `tests/` directory has more than 50 files (typical of `ext/standard/tests/`), which is too noisy for "primary" and the agent should rely on coupling history instead.
