...
- `impact_analysis` -- files affected by changing a symbol or file, with distance and reasons. Opt-in `include_forward` (forward deps), `include_siblings` (same-file symbols), `limit`, and `summary_only` controls; result is an object with `results` plus the requested extras. Optional `bounded: true` discloses incomplete Rust module resolution.
- `export_context` -- curated code bundle for a query or symbol, optionally with callers/callees; optional `bounded: true` discloses incomplete Rust module resolution in related context.
