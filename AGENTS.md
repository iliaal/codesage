...
- `project_overview` -- one bounded first-call orientation: languages, structural + semantic freshness, feature summary by kind, top-risk files, trust-boundary clusters, per-language test conventions, sample entrypoints, and suggested next calls. Aggregates indexed facts and computes risk analysis without model inference. The daemon shares a generation-validated top-50 ranking with `session_start`; overview shows ten rows, and freshness remains per request. Call once at session start. `hook_health` (`installed_hooks`, `last_run`, `last_exit`, `lock {state, pid, since, age_secs, reaped}`) reports the indexing-hook lock state; absent when the project is not a Git repository; the overview never reaps, `codesage doctor` does.
