...
- `find_references` -- references to a symbol; each row's `from_symbol` names the enclosing caller (null at file scope). Rows carry `from` (`sym:` handle of the enclosing symbol, omitted at file scope) and `to` (`sym:` handle of the definition the callsite resolves to with qualified, same-file, or import evidence; omitted otherwise). `to` resolution is bounded (256 distinct caller/name pairs, 750 ms); `to_resolution {resolved_pairs, capped}` appears only when the bound was hit. `lazy: true` marks a function-body import. The envelope carries `counts_floor: true` (name-based edges; an empty result means none found, not none exist), `definition_count`, and `ambiguous` with a `note` when several same-named definitions make the rows a union
