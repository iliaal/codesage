...
- **F4 Persisted signatures.** The parser computes signatures only transiently inside `edit_check` (`edit_check.rs:225-279`). Persist `signature`, `arity`, `visibility`, `params` (JSON), `returns` on `symbols` (additive migration, `extraction=3`; `extraction=2` was consumed by the lazy-import and visibility extraction shipped in 0.32.0). Cards and `find_symbol` show them; `edit_check` compares against stored HEAD signatures without re-parsing; "public API surface of this file" becomes a card section. Schema migration and public output change: human gate before implementation.
